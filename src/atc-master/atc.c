...
//####################################################################################################
void atc_init(atc_t *atc, const char *name, uint8_t usartx, void (*found)(char *))
{
  if (atc->inited == true)
    return;
...
bool atc_lock(atc_t *atc, uint32_t wait_ms)
{
  ...
  uint32_t start = bsp_GetFreeRunTime();
  while (bsp_GetFreeRunTime() - start < wait_ms)
  {
    atc_delay(1);
    if (atc->lock == 0)
    {
...
{
  LpUartSendData(data, len);
} 
//####################################################################################################
void atc_rxCallback(uint8_t data)
{
...
{
  if ((atc->rxIndex > 0) && (bsp_GetFreeRunTime() - atc->rxTime) > _ATC_RXTIMEOUT_MS)
    return true;
  return false;
}
//####################################################################################################
                   ...
                   int items, ...)
{
  ...
  uint32_t start = bsp_GetFreeRunTime();
  while (bsp_GetFreeRunTime() - start < timeout_ms)
  {
    atc_delay(1);
...
void atc_loop(atc_t *atc)
{
  ...
  if (bsp_GetFreeRunTime() - atc->loopTime < _ATC_RXTIMEOUT_MS)
  {
#if (_ATC_RTOS > 0)
    atc_delay(1);
#endif
  ...
  atc->loopTime = bsp_GetFreeRunTime();
