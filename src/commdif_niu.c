            ...
            }else
            {
                ...
                }else
                {
                    ...
                    for(i = 0;i<length;i++)
                    {
                        ...
                        // if(Addr>209)
                        // {
                        //     SEGGER_RTT_printf(0,"%02x",NiuMdAckTempBuf[i]);
                        // }
                    }
                    // if(Addr>209)
                    // {
...
{
    if(rdata == NIU_MODBUS_START_ID && NiurxIndex == 0)
    {
        NiuMdOrgInBuf[NiurxIndex++] = rdata;
        return;
    }else
    if(NiurxIndex > 0)
    {           
        ...
        {
            if(NiurxIndex == (NiuMdOrgInBuf[5]+8) && rdata == NIU_MODBUS_END_ID)
            {
                //接收完完整一帧
                //68 31 ce 68 02 02 60 6a 9d 16
                //SEGGER_RTT_printf(0,"iap recv:\n"); 
                // for(int i = 0;i<NiurxIndex;i++){
